...
import { getBallBySlug, balls } from '@/data/balls';
...
export async function generateMetadata({
  ...
  const ball = getBallBySlug(params.slug);
  if (!ball) {
    return {
      title: 'Ball nicht gefunden — Padel-Pilot.de',
    };
  }
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'article',
      locale: 'de_DE',
      images: [{ url: ball.image, alt: ball.imageAlt }],
    },
    twitter: {
    ...
    alternates: {
      canonical: `/baelle/${ball.slug}`,
    },
  };
}

function buildJsonLd(ball: ReturnType<typeof getBallBySlug> extends infer T ? NonNullable<T> : never) {
  return {
    '@context': 'https://schema.org',
    '@type': 'Product',
    name: `${ball.brand} ${ball.name}`,
    image: ball.image,
    description: ball.teaser,
    brand: {
      '@type': 'Brand',
      name: ball.brand,
    },
    ...
    review: {
      '@type': 'Review',
      reviewBody: ball.review.verdict.join(' '),
      author: {
        '@type': 'Organization',
        name: 'Padel-Pilot.de',
      },
...
const reviewSections = [
  ...
  { key: 'durability', heading: 'Haltbarkeit' },
...
export default function BallReviewPage({
  ...
  const jsonLd = buildJsonLd(ball);
  const scorePercent = (ball.score / 10) * 100;

  const pills = [
    { icon: Gauge, label: ball.type },
    { icon: Droplet, label: ball.pressure },
    { icon: Target, label: ball.level },
  ];
                  ...
                  {reviewSections.map((section) => (
                    <div key={section.key}>
                      ...
                      <div className="mt-4 space-y-4">
                        {ball.review[section.key].map((para, i) => (
                          <p
                            key={i}
                ...
                <div className="mt-12 flex flex-col items-start gap-4 rounded-2xl border border-border bg-secondary/30 p-6 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      Prüfe den aktuellen Preis des {ball.brand} {ball.name}.
                    </p>
                  </div>
                  <a
